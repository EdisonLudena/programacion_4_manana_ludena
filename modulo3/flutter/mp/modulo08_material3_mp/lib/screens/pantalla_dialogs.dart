...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo conectar a la base de fichajes'
            : 'Fichaje registrado correctamente en la plantilla'),
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas eliminar a Lionel Messi de la plantilla?\n'
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Jugador eliminado correctamente')),
      );
    }
  }
  ...
  Future<void> _mostrarConfirmacion2(BuildContext context) async {
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Confirmando que sirve')),
      );
    }
  }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
        ...
        content: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                controller: ctrlIp,
                decoration: const InputDecoration(labelText: 'IP del Vestuario'),
                validator:  (v) {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Jugador "${ctrlNombre.text}" agregado')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:           const Text('Avisos y Diálogos'),
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar jugador (confirmación)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
            onPressed: () => _mostrarFormulario(context),
            child: const Text('Agregar jugador (formulario)'),
          ),
